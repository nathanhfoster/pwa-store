...
import { SET_IN_DROP_ZONE, ADD_VALIDATED_FILE_LIST, DELETE_FILE_FROM_LIST, SET_ERROR } from '../types';
...
describe('FileUploadReducer', () => {
  const runTest = ({ name, props = {}, state = getInitialState(props), action, expectedState }, testNumber) => {
    const testNamePrefix = `Test ${testNumber}`;
    return it(
      name ? `${testNamePrefix} ${name} reducer` : `${testNamePrefix} should handle ${action.type} action type`,
      () => {

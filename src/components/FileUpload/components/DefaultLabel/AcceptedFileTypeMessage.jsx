...
import { getStylesFromProps, LIGHT_GREY, GREY, getAcceptedFileTypeMessage, formatBytes } from '../../utils';
import connect from '../../state/connect';

const useStyles = makeStyles({
  acceptedFileTypeMessage: ({ acceptedFileTypeMessageClassFromProps, disabled }) =>
    getStylesFromProps(acceptedFileTypeMessageClassFromProps, {
...
const AcceptedFileTypeMessage = (props) => {
  ...
  const fileSizeLimitMessage = useMemo(
    () =>
      fileSizeLimit
        ? `${acceptedFileTypeMessage ? ', u' : 'U'}p to ${formatBytes(fileSizeLimit)}${pluralFiles ? ' each' : ''}`
        : '',
...
const mapStateToProps = ({ classes: { acceptedFileTypeMessage }, accept, fileLimit, fileSizeLimit, disabled }) => ({

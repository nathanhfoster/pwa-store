...
const useStyles = makeStyles({
  errorMessagesListContainer: ({ errorMessagesListContainerClassFromProps }) =>
    getStylesFromProps(errorMessagesListContainerClassFromProps, {
      maxWidth: '100%',
      '& h4': {
        marginBottom: 0
      }
    }),
  errorMessageHeader: ({ errorMessageHeaderClassFromProps }) =>
    getStylesFromProps(errorMessageHeaderClassFromProps, {
      marginTop: 0
    }),
  errorMessagesList: ({ errorMessagesListClassFromProps }) =>
    getStylesFromProps(errorMessagesListClassFromProps, {
      '& ol': {
        marginBlock: 0,
        whiteSpace: 'nowrap',
        overflow: 'hidden',
  ...
  const renderListOfFiles = (errorMessage) => {
    ...
    return (
      <div key={errorId} className={classes.errorMessagesListContainer}>
        <h4 classes={classes.errorMessageHeader}>{message}:</h4>

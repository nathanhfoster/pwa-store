...
import { getStylesFromProps, RED, FILE_IMAGE_HEIGHT, FILE_IMAGE_WIDTH, LIGHT_GREY, preventDefaults } from '../../utils';
...
const useStyles = makeStyles({
  fileWrapper: ({ file, fileWrapperClassFromProps }) =>
  ...
  deleteIcon: ({ deleteIconClassFromProps }) =>
    getStylesFromProps(deleteIconClassFromProps, {
      fill: 'white',
      color: 'white',
      position: 'absolute',
      textAlign: 'center',
...
const FileImage = (props) => {
  const { file, fileIndex, previewImage, cantDeleteFile, onDelete, DeleteFileFromList } = props;
  ...
  return (
    <Fragment>
      {!cantDeleteFile && (
        <DeleteForever data-testid={`deleteIcon-${testId}`} className={classes.deleteIcon} onClick={handleDeleteFile} />
      )}
      {file.href ? (
        <a className={classes.fileWrapper} href={file.url} target='_blank' rel='noreferrer noopener'>
          {previewImage}
        </a>
      ) : (
        <div
...
const mapStateToProps = ({ fileList, classes: { fileWrapper, deleteIcon }, onDelete }, { fileIndex }) => {

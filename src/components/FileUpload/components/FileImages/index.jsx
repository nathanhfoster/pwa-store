...
import { AttachFile, Audiotrack, Description, Videocam } from '@material-ui/icons';
import FileImage from './FileImage';
import { getStylesFromProps, FILE_IMAGE_WIDTH, BLUE, OPAQUE, isImage, isPdf } from '../../utils';
import { fileListPropType } from '../../state/propTypes';
import connect from '../../state/connect';

const useStyles = makeStyles({
  fileImageItem: ({ hasADisabledOnDelete, fileImageItemClassFromProps }) =>
  ...
  file: ({ fileClassFromProps }) =>
    getStylesFromProps(fileClassFromProps, {
      display: 'flex',
      flexWrap: 'wrap',
      alignItems: 'center',
      height: 'auto',
  ...
  fileName: ({ fileNameClassFromProps }) =>
    getStylesFromProps(fileNameClassFromProps, {
      overflow: 'hidden',
      maxWidth: FILE_IMAGE_WIDTH,
      textOverflow: 'ellipsis'
    }),
  fileImage: ({ fileImageClassFromProps }) =>
    getStylesFromProps(fileImageClassFromProps, {
      height: '100%',
      width: '100%',
...
const FileImages = (props) => {
  ...
  return fileList.map((file, i) => {
    ...
    if (isImage(file) && src) {
      previewImage = <img className={classes.fileImage} title={file.name} name={file.name} alt={file.name} src={src} />;
...
const mapStateToProps = ({
  classes: { fileImageItem, file, fileName, fileImage },
  fileList,
  onDelete,
  showFileNames
}) => ({
